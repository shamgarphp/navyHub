        ...
        <div class="portlet light bordered">
          <div class="portlet-title">
            ...
            <div class="btn-group" style="float: right;">
               <a class="pull-right btn btn-warning btn-large" style="margin-right:40px" href="<?php echo site_url(); ?>/navy/submarine/AGMSReportexcel"><i class="fa fa-file-excel-o"></i> Export to Excel</a>
            </div>
          </div>
              ...
              <tbody>
                ...
                <!--  
                  ...
                  <td><?php if(!empty($agms['dtandtm'])) { echo ucfirst($agms['dtandtm']); } else { echo '---'; }?></td>
                   <td><?php if(!empty($agms['uname'])) { echo $agms['uname']; } else { echo '---'; }?></td>
                  <td><?php if(!empty($agms['area']) && !empty($agms['area'])) { echo $agms['area']; } else { echo '---'; }?></td>
                 <td><?php if(!empty($agms['location']) && !empty($agms['location'])) { echo $agms['location']; } else { echo '---'; }?></td>
                 <td><?php if(!empty($agms['latitude']) && !empty($agms['latitude'])) { echo $agms['latitude']; } else { echo '---'; }?></td>
                 <td><?php if(!empty($agms['longitude']) && !empty($agms['longitude'])) { echo $agms['longitude']; } else { echo '---'; }?></td>
                 <td><?php if(!empty($agms['rtype']) && !empty($agms['rtype'])) { echo $agms['rtype']; } else { echo '---'; }?></td>
                 <td><?php if(!empty($agms['channel_no']) && !empty($agms['channel_no'])) { echo $agms['channel_no']; } else { echo '---'; }?></td>
                 <td><?php if(!empty($agms['radiation_level']) && !empty($agms['radiation_level'])) { echo $agms['radiation_level']; } else { echo '---'; }?></td>
                 <td><?php if(!empty($agms['unit']) && !empty($agms['unit'])) { echo $agms['unit']; } else { echo '---'; }?></td>
                  <td>
                    <div class="btn-group">
